      ...
      <div className="grid gap-6 lg:grid-cols-3">
        {branches.map((branch) => (
          <div key={branch.slug} className="overflow-hidden rounded-card border border-border/60 bg-white shadow-[var(--shadow-soft)]">
            <FoodImage name={branch.name} icon="hall" className="h-40 w-full" />
            <div className="p-5">
              <h3 className="font-display text-lg text-charcoal">{branch.name}</h3>
              ...
              <div className="mt-4 flex gap-2">
                <Link href={`/locations/${branch.slug}`} className="focus-ring flex-1 rounded-full bg-charcoal px-3 py-2 text-center text-xs font-semibold text-white hover:bg-soft-black">
                  View Branch
                </Link>
                <Link href={`/menu?branch=${branch.slug}`} className="focus-ring flex-1 rounded-full border border-border px-3 py-2 text-center text-xs font-semibold text-charcoal hover:border-charcoal">
                  See Menu
                </Link>
              </div>
            </div>
